...
// No libraries. Canvas + simple physics.
// Flippers are "mallet paddles" that apply a directional impulse when pressed.
...
// Two mallet flippers (simplified as line segments that kick on press)
const flippers = {
  left: {
...
// Optional: tap areas for mobile testing
...
function onBankHit(){
  bankHits += 1;
  // Multipliers: 0-1 hits x1, 2 hits x2, 4 hits x4, 7 hits x8
...
function decayBanks(){
  // gentle decay so you keep moving
  if (bankHits > 0) bankHits -= 1;
  if (bankHits < 2) mult = 1;
  else if (bankHits < 4) mult = 2;
  else if (bankHits < 7) mult = 4;
  else mult = 8;
  setHUD();
}

// Physics constants
const GRAV = 760;      // px/s^2
const AIR  = 0.995;    // damping
const REST = 0.88;     // bounce
const MAXS = 1200;     // clamp velocity
...
function update(dt){
  ...
  if (
    ...
    // "goal" - bounce it back up and reward
    puck.vy = -Math.abs(puck.vy) * 0.92;
    addScore(1000);
    // slight random so it does not repeat forever
    puck.vx += rand(-40, 40);
  }

  // Flipper collisions (circle vs segment) with kick bonus if pressed
  collideWithFlipper(flippers.left, dt);
  collideWithFlipper(flippers.right, dt);
...
function resetPuckAfterDrain(){
  puck.stuck = true;
  puck.launched = false;
  puck.vx = 0;
  puck.vy = 0;
  // decay a chunk when you drain
...
function collideWithFlipper(f, dt){
  ...
  if (dist2 < hitR*hitR){
    ...
    // kick when pressed (this is the mallet strike)
    if (f.pressed){
      puck.vx += nx * 420;
      puck.vy += ny * 420;
      addScore(40);
